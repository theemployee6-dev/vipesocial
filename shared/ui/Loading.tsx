...
export default function LoadingScreen({
  ...
  useEffect(() => {
    ...
    const animate = (ts: number) => {
      ...
      // Atualiza DOM diretamente — sem re-render para animação fluida
      if (liquidRef.current) {
        const ty = pct === 0 ? 100 : Math.min(100 - pct, 70);
        liquidRef.current.style.transform = `translateY(${ty}%)`;
      }
      if (pctRef.current) {
  ...
  // Modo controlado: atualiza DOM diretamente
  useEffect(() => {
    if (!isControlled) return;
  ...
  // fill só usado para bubbleOpacity (não causa jank — atualiza raramente)
    ...
    <>
      <style>{`
        ...
        .ls-liquid {
          ...
          right: 0;
          height: 100%;
          transform: translateY(100%); /* inicial — controlado via ref */
          background: linear-gradient(
            180deg,
            #00ffdd 0%,
            #00ff88 30%,
      ...
      <div className="ls-root">
        ...
        <div className="ls-mask">
          ...
          {/* Líquido */}
          <div ref={liquidRef} className="ls-liquid">
            {/* Ondas */}
        ...
        {/* Percentual */}
        <div ref={pctRef} className="ls-pct">0%</div>
      </div>
    </>
  );
}
