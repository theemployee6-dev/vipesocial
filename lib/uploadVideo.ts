...
import type { VideoMetricas, PerfilCriador } from "./core/domain/vipe.types";
...
// Instância do serviço de storage (poderia ser injetada, mas para simplificar criamos aqui)
...
export async function uploadVideoParaSupabase(
  file: File,
  ...
  const publicUrl = storageService.getPublicUrl("videos-virais", filePath);
  console.log("✅ [SUPABASE] Upload concluído. URL:", publicUrl);
...
export async function enviarParaApiAnalise(
  ...
  if (!response.ok) {
    ...
    console.error("❌ [API] Erro:", erroData);
    throw new Error(
      erroData.error || erroData.message || "Falha na resposta da IA",
    );
  }
